...
/**
 * Escritura del catálogo de categorías de gasto (tenant-wide). Compartido por
 * empresa y sucursal — el catálogo es el mismo; el admin-sucursal se gatea con
 * el toggle `branch_admin_expense_categories_enabled`. El borrado (destroy) NO
 * vive aquí: queda reservado a empresa/superadmin.
 *
 * Requiere que el controlador anfitrión exponga `$this->aiDrafts`
 * (App\Services\Ai\AiCategoryDraftService) vía su constructor.
 */
trait HandlesExpenseCategoryWrites
{
    ...
    public function storeFromAiDraft(Request $request): JsonResponse
    {
        ...
        $category = DB::transaction(function () use ($tenant, $userId, $validated, $subcategories, $draft, $mode) {
            ...
            // Crear subcategorías nuevas (válido para ambos modos). Validamos
            // duplicados contra las subcategorías ya existentes de la categoría
            // destino.
