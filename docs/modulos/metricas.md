...
>
> **Nota (junio 2026 — Resumen):** El "Resumen" (index) dejó de ser solo un hub: ahora es una **pantalla con datos** que arma `App\Services\Metrics\OverviewMetrics` y entrega como prop `data`. Muestra **P&L** (Ventas − **Costo de lo vendido (CMV real)** = Utilidad bruta − Gastos = **Utilidad neta**) con **cobertura de costo** honesta (por conteo de items), **KPIs hero con comparación vs período anterior** (`DateRange::previousComparable()`), **Compras del período** como dato de caja aparte (no se resta), **alertas** derivadas de datos (cobranza vencida, margen bajo, gastos al alza, sucursal a la baja), **top productos por utilidad** y **comparativa por sucursal** (empresa). El grid de ejes (`MetricsHubGrid`) se conserva debajo, en "Explorar a fondo". Spec: [`docs/superpowers/specs/2026-06-18-metricas-resumen-utilidad-design.md`](../superpowers/specs/2026-06-18-metricas-resumen-utilidad-design.md). Esto **supersede** lo que dice este doc abajo sobre que el Resumen "no transporta `data`".
