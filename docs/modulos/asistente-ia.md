...
| `preparar_borrador_cliente` | `PrepareCustomerDraftTool` | Cliente | Alta de cliente (incluye cajero, **si su sucursal tiene `cashier_customers_enabled`**); branch forzado para sucursal/cajero, empresa elige sucursal; detecta duplicados por nombre difuso |
...
| `preparar_cobro_cliente` | `PrepareCustomerPaymentDraftTool` | Cobro global a cliente (FIFO) | Resuelve cliente por nombre (candidatos explícitos si es ambiguo); el desglose FIFO lo calcula `CustomerGlobalPaymentService::preview()`. **Confirmar exige turno abierto** y que el cliente sea de la sucursal del turno (D2); `apply()` re-calcula la distribución al confirmar. Para el cajero exige además `cashier_customers_enabled` en su sucursal — mismo gate que `/caja/clientes`, ver [clientes-caja.md](clientes-caja.md) |
...
- **Quick actions (F4):** chips de acción sugerida (`resources/js/Components/Asistente/app/QuickActions.vue`) bajo la última card de resultados — envían prompts predefinidos ("Productos más vendidos", "Cobrar una deuda", "Pagar a un proveedor", …). Viven en `MessageThread` compartido.
