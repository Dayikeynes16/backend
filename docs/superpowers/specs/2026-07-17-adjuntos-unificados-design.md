...
- **`CompraFormModal.vue`**: reemplaza el `<input type="file">` nativo por `AttachmentsPicker` con `mode="staged"` — gana cámara, miniaturas y visor por primera vez. Nota para el plan: hoy `CompraFormModal.vue` solo recibe `routes: {store, update}` como prop y **no muestra los adjuntos ya existentes en modo edición, para ningún rol** — a diferencia de `GastoFormModal.vue`, que sí recibe `attachmentPreviewRouteName`/`attachmentDownloadRouteName`/`attachmentDestroyRouteName` y renderiza `existingAttachments`. Hay que agregarle esas mismas props (u equivalentes) al componente, y wirearlas en **las tres páginas que lo invocan**:
  - `Pages/Empresa/Compras/Index.vue` y `Pages/Sucursal/Compras/Index.vue`: agregar `attachmentPreviewRouteName`/`attachmentDownloadRouteName`/`attachmentDestroyRouteName` a su `formRoutes` (hoy solo `{store, update}`), apuntando a `empresa.compras.adjuntos.preview/download/destroy` y `sucursal.compras.adjuntos.preview/download/destroy` respectivamente — **rutas que ya existen**, sin cambios de backend en estos dos roles (mismo patrón que `CompraDetailModal.vue` ya usa correctamente ahí).
  - `Pages/Caja/Compras/Index.vue`: igual, pero con las 3 rutas nuevas de Caja (ver sección siguiente, ahí sí hay backend nuevo).
...
1. **`Pages/Caja/Compras/Index.vue`**: el objeto `cajaCompraRoutes` que se le pasa a `CompraDetailModal` como prop `routes` solo trae `{ cancel, pagoStore, pagoDestroy }` — **le faltan** `adjuntoDownload`, `adjuntoPreview`, `adjuntoDestroy`. En la práctica hoy no truena: el bloque de adjuntos de `CompraDetailModal.vue` está detrás de `v-if="purchase.attachments?.length && routes.adjuntoPreview"` (línea 211), así que simplemente **nunca se muestra** para un cajero — no hay error visible, solo la sección desaparece en silencio.
2. **`Pages/Caja/Gastos/Index.vue`**: pasa a `GastoDetailModal` `preview-route-name="caja.gastos.index"`, `download-route-name="caja.gastos.index"` y a `GastoFormModal` `attachment-destroy-route-name="caja.gastos.store"` — **nombres de ruta equivocados** (apuntan al listado y al alta de gastos, no a rutas de adjuntos). Parece un copy-paste sin las rutas reales, porque **no existen** rutas `caja.gastos.adjuntos.*` todavía. Además, `GastoFormModal` **ni siquiera recibe** `attachment-preview-route-name`/`attachment-download-route-name` hoy (estas props no están wireadas en absoluto) — y su bloque de adjuntos existentes está gateado por `v-if="attachmentPreviewRouteName"`, así que sin agregarlas, un cajero editando un gasto tampoco vería sus adjuntos ya subidos en el formulario (solo en el modal de detalle).
...
- **Ver/descargar/previsualizar** (`authorizeView`, usado por `download()`/`preview()`): cajero limitado a **su propia sucursal y sus propios registros** en ambos módulos — `expense.user_id === $user->id` para Gastos (`Caja\GastoController@index` filtra por `branch_id`+`user_id`) y `purchase.created_by === $user->id` para Compras (`Caja\PurchaseController@index` filtra por `branch_id`+`created_by` — **corrección de un error de esta spec**: se había asumido que Compras no filtraba por dueño y mostraba toda la sucursal; la UI de Caja incluso lo dice explícito, "Solo ves las compras que tú registraste" (`Pages/Caja/Compras/Index.vue` línea 76). Sin esta corrección, un cajero habría podido ver/descargar facturas de compras registradas por otros cajeros de su misma sucursal).
- **Eliminar** (`authorizeMutation`, usado por `destroy()`): además de lo anterior, exige que el registro esté ligado al turno abierto del cajero — mismo campo que ya calcula `can_manage` en los índices: `expense.cash_register_shift_id === $turnoAbierto->id` (Gastos) / `purchase.cash_register_shift_id === $turnoAbierto->id` (Compras). Por defensa en profundidad, también revalida `created_by`/`user_id` (aunque en la práctica el turno abierto ya implica ser el dueño). Admin-sucursal/superadmin no cambian (acceso total a su sucursal / cualquiera).
- **Nota (fuera de alcance):** ninguno de los dos controladores valida hoy los flags de sucursal `cashier_expenses_enabled`/`cashier_purchases_enabled` (que sí gatean el resto del módulo vía `ensureModuleEnabled()`). Se deja así — un cajero conservando acceso de solo-lectura a adjuntos de registros que ya existían antes de que el módulo se deshabilitara para su sucursal es aceptable y consistente con cómo se comporta hoy el resto de la vista de detalle; si se quiere cerrar también ese caso, es un ajuste aparte.
...
- `Pages/Caja/Gastos/Index.vue`: corregir `preview-route-name`/`download-route-name` en `GastoDetailModal` y `attachment-destroy-route-name` en `GastoFormModal` a las 3 rutas nuevas de arriba (hoy apuntan, por error, a rutas de listado/alta). **Además**, agregar a `GastoFormModal` las props `attachment-preview-route-name="caja.gastos.adjuntos.preview"` y `attachment-download-route-name="caja.gastos.adjuntos.download"` — hoy no existen en absoluto en este wiring, y sin ellas el formulario de edición seguiría sin mostrar los adjuntos ya subidos (el mismo `v-if="attachmentPreviewRouteName"` que ya gatea esa sección en Empresa/Sucursal). Sin este paso, Gastos quedaría con paridad de Caja incompleta frente a Compras.
- `CompraFormModal.vue` en modo Caja gana las mismas props de adjuntos que se agregan también para Empresa/Sucursal (ver "Cambios por consumidor" arriba) — mismo componente en los tres roles, la única diferencia es qué nombres de ruta se le pasan.
...
- No toca las rutas ni la validación backend de adjuntos de Gastos/Compras para Empresa/Sucursal (mismos endpoints `store`/`destroy`/`preview`/`download` de siempre).
...
**Riesgo adicional (autorización):** `ExpenseAttachmentController::authorizeAccess()` y `PurchaseAttachmentController::authorizeAccess()` ya están en producción sirviendo a admin-empresa/admin-sucursal. Separarlos en `authorizeView`/`authorizeMutation` para poder acotar al cajero **no debe cambiar el comportamiento actual de esos dos roles** — la migración es un split mecánico (mover el cuerpo existente a `authorizeView`, agregar el chequeo de turno solo dentro de `authorizeMutation`, sin tocar la rama `admin-sucursal`/`superadmin`). La regresión de esto se cubre en "Pruebas" con la suite completa + tests explícitos de que admin-empresa/admin-sucursal siguen accediendo igual que antes.
