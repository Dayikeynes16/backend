...
// apaga el primero, el segundo se apaga junto con él (y queda deshabilitado
// en el template mientras tanto).
watch(() => form.payment_receipts_enabled, (enabled) => {
    if (!enabled) {
...
<template>
    ...
    <EmpresaLayout>
        ...
        <form @submit.prevent="submit" class="mx-auto max-w-3xl space-y-6 pb-12">
            ...
            <section class="rounded-xl bg-white shadow-sm ring-1 ring-gray-100">
                <div class="border-b border-gray-100 px-6 py-5">
                    <h2 class="text-base font-bold text-gray-900">Permisos del cajero</h2>
                    <p class="mt-1 text-sm text-gray-500">Controla qué puede registrar el cajero desde la caja. El pago en efectivo de gastos y compras sale del cajón y afecta su corte de turno.</p>
                </div>
                <div class="divide-y divide-gray-100">
                    <div class="flex items-start justify-between gap-4 px-6 py-5">
                        <div>
                            <p class="text-sm font-bold text-gray-800">Registrar gastos</p>
                            <p class="mt-0.5 text-xs text-gray-500">Muestra el módulo de Gastos al cajero y permite registrar gastos en efectivo durante su turno.</p>
                        </div>
                        <label class="inline-flex shrink-0 cursor-pointer items-center gap-2">
                            <input type="checkbox" v-model="form.cashier_expenses_enabled" class="peer sr-only" />
                            <div class="relative h-6 w-11 rounded-full bg-gray-200 transition peer-checked:bg-red-600">
                                <div class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition peer-checked:translate-x-5"></div>
                            </div>
                            <span class="text-sm font-semibold text-gray-700">{{ form.cashier_expenses_enabled ? 'Activo' : 'Inactivo' }}</span>
                        </label>
                    </div>
                    <div class="flex items-start justify-between gap-4 px-6 py-5">
                        <div>
                            <p class="text-sm font-bold text-gray-800">Registrar compras</p>
                            <p class="mt-0.5 text-xs text-gray-500">Muestra el módulo de Compras al cajero y permite registrar compras pagadas en efectivo durante su turno.</p>
                        </div>
                        <label class="inline-flex shrink-0 cursor-pointer items-center gap-2">
                            <input type="checkbox" v-model="form.cashier_purchases_enabled" class="peer sr-only" />
                            <div class="relative h-6 w-11 rounded-full bg-gray-200 transition peer-checked:bg-red-600">
                                <div class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition peer-checked:translate-x-5"></div>
                            </div>
                    ...
                    <div class="flex items-start justify-between gap-4 px-6 py-5">
                        <div>
                            <p class="text-sm font-bold text-gray-800">Clientes y cobros</p>
                            <p class="mt-0.5 text-xs text-gray-500">Muestra el módulo de Clientes al cajero: consultar la cartera, dar de alta o editar clientes y registrar cobros de cuentas pendientes durante su turno. Los precios preferenciales siguen siendo exclusivos del admin de sucursal.</p>
                        </div>
                        <label class="inline-flex shrink-0 cursor-pointer items-center gap-2">
                            <input type="checkbox" v-model="form.cashier_customers_enabled" class="peer sr-only" />
                            <div class="relative h-6 w-11 rounded-full bg-gray-200 transition peer-checked:bg-red-600">
                                <div class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition peer-checked:translate-x-5"></div>
                            </div>
            ...
            <section class="rounded-xl bg-white shadow-sm ring-1 ring-gray-100">
                <div class="border-b border-gray-100 px-6 py-5">
                    <h2 class="text-base font-bold text-gray-900">Comprobantes de pago</h2>
                    <p class="mt-1 text-sm text-gray-500">Controla si se puede (o debe) adjuntar comprobante en los cobros por transferencia.</p>
                </div>
                <div class="divide-y divide-gray-100">
                    <div class="flex items-start justify-between gap-4 px-6 py-5">
                        <div>
                            <p class="text-sm font-bold text-gray-800">Comprobantes de transferencia</p>
                            <p class="mt-0.5 text-xs text-gray-500">Permite adjuntar el comprobante (imagen o PDF) a los cobros por transferencia.</p>
                        </div>
                        <label class="inline-flex shrink-0 cursor-pointer items-center gap-2">
                            <input type="checkbox" v-model="form.payment_receipts_enabled" class="peer sr-only" />
                            <div class="relative h-6 w-11 rounded-full bg-gray-200 transition peer-checked:bg-red-600">
                                <div class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition peer-checked:translate-x-5"></div>
                            </div>
                            <span class="text-sm font-semibold text-gray-700">{{ form.payment_receipts_enabled ? 'Activo' : 'Inactivo' }}</span>
                        </label>
                    </div>
                    <div class="flex items-start justify-between gap-4 px-6 py-5">
                        <div>
                            <p class="text-sm font-bold text-gray-800">Exigir comprobante</p>
                            <p class="mt-0.5 text-xs text-gray-500">No se podrá cobrar por transferencia sin adjuntar el comprobante.</p>
                        </div>
                        <label
                            class="inline-flex shrink-0 items-center gap-2"
                            :class="form.payment_receipts_enabled ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'">
                            <input
                                type="checkbox"
                                v-model="form.payment_receipts_required"
                                :disabled="!form.payment_receipts_enabled"
                                class="peer sr-only" />
                            <div class="relative h-6 w-11 rounded-full bg-gray-200 transition peer-checked:bg-red-600">
                                <div class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition peer-checked:translate-x-5"></div>
                            </div>
