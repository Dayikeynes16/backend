...
/**
 * Cartera de clientes vista desde Caja. Misma pantalla que la de sucursal sin
 * lo que no es del cajero: no elimina clientes, no cambia su estado y no ve la
 * columna de precios preferenciales (los descuentos son del admin-sucursal).
 */
...
const applyFilters = () => {
    ...
    debounceTimer = setTimeout(() => {
        router.get(
            route('caja.clientes.index', props.tenant.slug),
            {
...
const submitCreate = () => {
    createForm.post(route('caja.clientes.store', props.tenant.slug), {
        preserveScroll: true,
...
const editForm = useForm({ name: '', phone: '', notes: '' });
...
const submitEdit = () => {
    ...
    editForm.put(route('caja.clientes.update', [props.tenant.slug, editingCustomer.value.id]), {
        preserveScroll: true,
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="c in customers.data" :key="c.id" class="group">
                                <td class="whitespace-nowrap px-5 py-3.5">
                                    <Link :href="route('caja.clientes.show', [tenant.slug, c.id])"
