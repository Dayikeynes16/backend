...
const startEditPayment = (p) => { editingPaymentId.value = p.id; editPaymentForm.method = p.method; editPaymentForm.amount = parseFloat(p.amount); };
const submitEditPayment = (paymentId) => {
    editPaymentForm.put(route('sucursal.workbench.payment.update', [props.tenant.slug, selected.value.id, paymentId]), {
        preserveScroll: true, onSuccess: () => { editingPaymentId.value = null; },
    });
};
...
const addToCart = (product, presentation = null) => {
    ...
    cart.value.push({ key, product_id: product.id, presentation_id: presentation?.id || null, name, price, unit_type: product.unit_type, sale_mode: product.sale_mode, quantity: product.sale_mode === 'weight' ? 0 : 1, image_path: product.image_path });
};
                            ...
                            <div class="space-y-1.5">
                                <div v-for="p in selected.payments" :key="p.id" class="rounded-lg bg-gray-50 px-4 py-2.5">
                                    <form v-if="editingPaymentId === p.id" @submit.prevent="submitEditPayment(p.id)" class="flex items-center gap-3">
                                        ...
                                        <div class="relative flex-1"><span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-400">$</span>
                                        <input v-model="editPaymentForm.amount" type="number" step="0.01" min="0.01" required class="block w-full rounded-lg border-gray-200 pl-7 text-sm focus:border-red-400 focus:ring-red-300" /></div>
                                    ...
                                    <div v-else class="flex items-center justify-between">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <template v-if="canEditPayments">
                                                ...
                                                <button @click="confirmDeletePayment = p.id" class="text-xs text-gray-400 hover:text-red-600">Eliminar</button>
                                            </template>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex-1 overflow-y-auto p-4">
                                    ...
                                    <div v-else class="space-y-2">
                                        <div v-for="(item, idx) in cart" :key="idx" class="flex items-center gap-3 rounded-lg bg-gray-50 px-4 py-3">
                                            <div class="min-w-0 flex-1"><p class="truncate text-sm font-semibold text-gray-900">{{ item.name }}</p><p class="text-xs text-gray-400">${{ item.price.toFixed(2) }}</p></div>
