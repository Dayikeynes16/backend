...
// Emparejamiento pedido web ↔ venta de báscula. Admin/cajero abre el modal
// desde una venta de báscula activa para vincularla con un pedido web pendiente.
...
const submitUnlink = () => {
    ...
    router.delete(
        route('sucursal.workbench.unlink-order', [props.tenant.slug, selected.value.id]),
        {
            preserveScroll: true,
            preserveState: true,
            onFinish: () => {
...
const {
    ...
    sale: () => selected.value,
    linkUrl: () => route('sucursal.workbench.whatsapp-link', [props.tenant.slug, selected.value.id]),
    savePhoneUrl: () => route('sucursal.workbench.whatsapp-phone', [props.tenant.slug, selected.value.id]),
    deletePhoneUrl: () => route('sucursal.workbench.whatsapp-phone.destroy', [props.tenant.slug, selected.value.id]),
            ...
            <div class="flex flex-1 flex-col rounded-xl bg-white shadow-sm ring-1 ring-gray-100">
                ...
                <template v-else>
                    ...
                    <div class="border-b border-gray-100 px-6 py-4">
                        ...
                        <!-- Phone chip: siempre visible para que el cajero sepa a qué número se enviará -->
                    ...
                    <!-- WhatsApp error banner (sólo cuando ningún diálogo está abierto) -->
                    <div v-if="whatsappError && !whatsappConfirmDialog.show && !whatsappCaptureDialog.show && !whatsappRemoveDialog.show"
                        class="border-b border-red-100 bg-red-50 px-6 py-2">
