...
const formData = reactive({
    ...
    unit_type: props.producto.unit_type || 'piece',
    image: null,
    _method: 'put',
});
    ...
    <SucursalLayout>
        <template #header>
            <div class="flex items-center gap-2 text-sm">
                ...
                <span class="font-bold text-gray-900">{{ producto.name }}</span>
            </div>
        </template>

        <form @submit.prevent="submit" class="mx-auto max-w-3xl space-y-8">
            <!-- 1. Información del Producto + Estado -->
            ...
            <section class="rounded-xl bg-white shadow-sm ring-1 ring-gray-100">
                <div class="border-b border-gray-100 px-6 py-5">
                    ...
                    <p class="mt-1 text-sm text-gray-500">JPG, PNG o WebP. Maximo 2MB.</p>
                </div>
                <div class="p-6">
                    <div v-if="imagePreview" class="space-y-3">
                        ...
                        <div class="flex gap-3">
                            <button type="button" @click="fileInput?.click()" class="text-sm font-semibold text-red-600 hover:text-red-700">Cambiar imagen</button>
                            <button type="button" @click="removeImage" class="text-sm text-gray-400 hover:text-gray-600">Quitar</button>
                        </div>
                    </div>
                    <div v-else @click="fileInput?.click()" class="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-200 px-6 py-12 transition hover:border-red-300 hover:bg-red-50/30">
                        ...
                        <p class="mt-3 text-sm font-medium text-gray-500">Haz click para seleccionar</p>
            ...
            <section class="rounded-xl border-2 border-red-200 bg-red-50">
                <div class="flex items-center justify-between px-6 py-5">
                    <div>
                        <p class="text-sm font-semibold text-red-900">Eliminar producto</p>
