...
const props = defineProps({
    ...
    /**
     * Cancelar un cobro es exclusivo del admin de sucursal: en Caja no existe
     * la ruta `destroy` y el backend la rechazaría igual.
     */
...
const submitCancel = async () => {
    ...
    try {
        const res = await fetch(
            route(`${props.routePrefix}.clientes.cobro-global.cancel`, [props.tenantSlug, props.customerId, props.customerPaymentId]),
            {
                method: 'DELETE',
                headers: {
...
const load = async () => {
    ...
    try {
        const res = await fetch(
            route(`${props.routePrefix}.clientes.cobro-global.show`, [props.tenantSlug, props.customerId, props.customerPaymentId]),
            {
...
<template>
    <Modal :show="show" max-width="2xl" @close="emit('close')">
        ...
        <div v-else-if="data" class="relative">
            ...
            <div class="flex items-center gap-3 border-t border-gray-100 bg-gray-50/60 px-7 py-4"
                :class="canCancel ? 'justify-between' : 'justify-end'">
                <button v-if="canCancel" @click="openCancelConfirm"
