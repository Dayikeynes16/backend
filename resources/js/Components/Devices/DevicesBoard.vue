...
const alertText = (d) => {
    if (d.status === 'battery_critical') return `${d.shown_name} está al ${d.battery_level} % y se va a apagar.`;
    ...
    <div class="space-y-5">
        <div v-if="alerts.length" class="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4">
            ...
            <ul class="mt-1.5 space-y-1 text-sm text-amber-800">
                <li v-for="d in alerts" :key="d.id" class="flex items-start gap-2" :class="{ 'text-red-700': d.status === 'battery_critical' }">
                    <svg :class="['mt-0.5 h-4 w-4 shrink-0', d.status === 'battery_critical' ? 'text-red-500' : 'text-amber-500']" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" /></svg>
