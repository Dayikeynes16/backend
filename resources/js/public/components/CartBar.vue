    ...
    <Transition name="slide-up">
        <div v-if="cart.count.value > 0" class="fixed bottom-4 left-4 right-4 z-40 sm:bottom-6 sm:left-auto sm:right-6 sm:w-96">
            <button @click="emit('open')"
                class="flex w-full items-center justify-between gap-4 rounded-full bg-red-600 py-4 pl-5 pr-5 text-left text-white shadow-xl transition hover:bg-red-700 active:scale-[0.98]">
