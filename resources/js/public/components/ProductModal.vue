        ...
        <div class="w-full max-w-lg overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl">
            ...
            <div class="p-5">
                ...
                <div v-if="product.presentations.length > 0 && ['presentation', 'both'].includes(product.sale_mode)" class="mt-5">
                    <p class="text-sm font-bold text-gray-900">Presentación</p>
                ...
                <div v-if="supportsAmountMode" class="mt-5">
                    <div class="grid grid-cols-2 gap-0 rounded-xl bg-gray-100 p-1">
                        <button type="button" @click="mode = 'qty'"
                            :class="['flex items-center justify-center gap-2 rounded-lg py-3 text-sm font-bold transition',
                                mode === 'qty' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600']">
                            ⚖️ Por kilos
                        </button>
                        <button type="button" @click="mode = 'amount'"
                            :class="['flex items-center justify-center gap-2 rounded-lg py-3 text-sm font-bold transition',
                                mode === 'amount' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600']">
                            💲 Por precio
                        </button>
                    </div>
                </div>
                ...
                <div v-if="mode === 'qty'" class="mt-5">
                    <p class="text-sm font-bold text-gray-900">Cantidad</p>
                    <div class="mt-2 flex items-center gap-3">
                        <button @click="decrement" type="button" aria-label="Disminuir cantidad"
                            class="flex h-12 w-12 items-center justify-center rounded-full bg-gray-100 text-gray-700 transition hover:bg-gray-200 active:bg-gray-300">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" /></svg>
                        </button>
                ...
                <div v-else class="mt-5">
                    <p class="text-sm font-bold text-gray-900">¿Cuánto quieres gastar?</p>

                    <!-- Big amount display with +/- -->
                    <div class="mt-2 flex items-center gap-3">
                        <button @click="bumpAmount(-50)" type="button" aria-label="Restar 50 pesos"
                            class="flex h-12 w-12 items-center justify-center rounded-full bg-gray-100 text-gray-700 transition hover:bg-gray-200 active:bg-gray-300">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" /></svg>
                        </button>
                        <div class="flex-1 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <input type="number" inputmode="numeric" v-model.number="amount" min="10" step="10"
                                    class="w-32 border-0 bg-transparent text-center text-4xl font-extrabold tabular-nums text-gray-900 focus:ring-0 focus:outline-none" />
                            </div>
                            <p class="mt-1 text-sm text-gray-500">≈ <span class="font-semibold text-gray-700">{{ derivedQuantity.toFixed(3) }} {{ unitLabel }}</span></p>
                        </div>
                        <button @click="bumpAmount(50)" type="button" aria-label="Sumar 50 pesos"
                            class="flex h-12 w-12 items-center justify-center rounded-full bg-gray-100 text-gray-700 transition hover:bg-gray-200 active:bg-gray-300">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                        </button>
                    </div>
                    ...
                    <!-- Helper text -->
                    <div class="mt-3 rounded-xl bg-amber-50 p-3.5 text-sm text-amber-900 ring-1 ring-amber-100">
