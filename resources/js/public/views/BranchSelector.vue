    ...
    <div class="min-h-screen bg-gradient-to-br from-red-50 via-white to-orange-50">
        ...
        <div v-else-if="error === 'not_found'" class="flex min-h-screen items-center justify-center p-6">
            <div class="text-center">
                ...
                <p class="mt-1 text-base text-gray-500">Verifica el link que te compartieron.</p>
            </div>
        </div>
        ...
        <div v-else-if="tenant" class="mx-auto max-w-xl px-5 py-10">
            <header class="text-center">
                ...
                <p class="mt-2 text-base text-gray-600">Elige la sucursal donde quieres hacer tu pedido.</p>
            </header>

            <div class="mt-6 flex justify-center">
                <button v-if="!userLocation && !askingLocation" @click="requestLocation"
                    class="inline-flex items-center gap-2 rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-red-700 shadow-sm ring-1 ring-red-200 transition hover:bg-red-50">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" /></svg>
                    Usar mi ubicación
                </button>
                <span v-else-if="askingLocation" class="text-sm text-gray-500">Obteniendo ubicación…</span>
            </div>

            <div class="mt-8 space-y-3">
                <button v-for="b in sortedBranches" :key="b.id" @click="goToMenu(b.id)"
                    class="group flex w-full items-start gap-4 rounded-2xl bg-white p-5 text-left shadow-sm ring-1 ring-gray-100 transition hover:ring-red-300 active:scale-[0.99]">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span v-if="b.distance_km != null" class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-600">a {{ b.distance_km.toFixed(1) }} km</span>
                        </div>
                        <p v-if="b.address" class="mt-1 truncate text-sm text-gray-600">{{ b.address }}</p>
                        <p v-if="b.schedule" class="mt-1 text-sm text-gray-500">{{ b.schedule }}</p>
