    ...
    <div class="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50">
        <div class="mx-auto max-w-lg px-5 py-10">
            <div class="text-center">
                ...
                <p class="mt-3 text-base text-gray-700">Tu pedido <span class="font-bold text-gray-900">{{ orderData?.folio || '#' + saleId }}</span> se registró correctamente.</p>
                <p class="mt-1 text-base text-gray-700">Total: <span class="font-bold text-gray-900 tabular-nums">${{ (orderData?.total || 0).toFixed(2) }}</span></p>
            </div>
            ...
            <div class="mt-8 rounded-3xl bg-white p-6 shadow-lg ring-1 ring-gray-100">
                ...
                <div v-if="orderData?.whatsapp_url" class="mt-5">
                    <button @click="openWhatsapp"
                        class="flex w-full items-center justify-center gap-3 rounded-2xl bg-green-500 py-4 text-base font-bold text-white shadow-lg transition hover:bg-green-600 active:scale-[0.98]">
            ...
            <div class="mt-8 text-center">
                <button @click="goBackToMenu" class="text-base font-semibold text-gray-600 underline hover:text-gray-900">Volver al menú</button>
            </div>
        </div>
    </div>
</template>
